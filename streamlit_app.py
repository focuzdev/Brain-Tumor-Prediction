...
MODEL_CONFIGS = [
    # name shown to user, filename, preprocessing style
    {"key": "custom_cnn", "label": "Custom CNN", "file": "brain_tumor_model.h5", "preprocess": "rescale"},
    {"key": "mobilenet",  "label": "MobileNetV2", "file": "mobilenet_model.h5", "preprocess": "mobilenet"},
]

@st.cache_resource(show_spinner=False)
def load_models():
    """
    Loads every .h5 model that is present next to this script.
    Returns (loaded: dict[key -> keras.Model], errors: dict[key -> str]).
    Cached so models are loaded once per session, not on every click.
    """
    loaded, errors = {}, {}
    if not TF_AVAILABLE:
    ...
    for cfg in MODEL_CONFIGS:
        ...
        if not os.path.exists(path):
            errors[cfg["key"]] = f"{cfg['file']} not found next to streamlit_app.py"
            continue
        try:
