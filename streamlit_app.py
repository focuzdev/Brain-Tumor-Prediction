...
def find_last_conv_layer(model):
    """Finds the last Conv2D-type layer in a (possibly nested) Keras model, for Grad-CAM."""
    for layer in reversed(model.layers):
        if isinstance(layer, tf.keras.layers.Conv2D):
            return layer.name
        # Handle nested models (e.g. a MobileNetV2 base wrapped in a Sequential/Functional model)
...
def generate_gradcam(pil_img, model, preprocess_style, pred_index):
    ...
    gradients, and produces a localization heatmap. Falls back to None if
    the model has no identifiable conv layer (caller should fall back to
    the synthetic heatmap and label it as such).
    """
...
/* Sidebar show/hide control -- Streamlit's default arrow is tiny and low-contrast;
   give it a real button look so it's easy to find in either theme. */
...
[data-testid="collapsedControl"] svg{{fill:#0284c7 !important;width:22px !important;height:22px !important}}
[data-testid="stSidebarCollapseButton"]{{
  background:{"#1e2d45" if _dk else "#ffffff"} !important;
  border:1.5px solid #38bdf8 !important;
  border-radius:8px !important;
}}
[data-testid="stSidebarCollapseButton"] svg{{fill:#0284c7 !important}}

/* Custom sidebar toggle -- doesn't rely on Streamlit's internal, version-
   dependent markup for the native collapse control. This is a floating
   button we fully own, always rendered in a fixed, predictable spot. */
...
st.markdown("""
<button id="ns-sb-toggle" title="Show/hide sidebar">☰</button>
<script>
(function(){
  function attach(){
    var btn = document.getElementById('ns-sb-toggle');
    if (!btn) return;
    function sync(){
      var collapsed = document.body.classList.contains('ns-sidebar-collapsed');
      btn.textContent = collapsed ? '\\u2630' : '\\u2715';
    }
    btn.onclick = function(){
      document.body.classList.toggle('ns-sidebar-collapsed');
      sync();
    };
    sync();
  }
  attach();
  // Streamlit re-renders parts of the DOM on every rerun; make sure our
  // button keeps working after that by re-attaching if it's ever replaced.
...
with st.sidebar:
    ...
    st.caption(f"Model mode: **{_mode}** "
               f"({'both models, higher memory use' if _mode == 'ensemble' else 'single model, lighter footprint'})")
