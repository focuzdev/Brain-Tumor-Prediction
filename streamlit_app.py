...
def _ensure_model_downloaded(cfg):
    ...
    url = st.secrets.get(f"{cfg['key'].upper()}_MODEL_URL", cfg.get("url")) if hasattr(st, "secrets") else cfg.get("url")
    if not url:
        return None, f"{cfg['file']} is missing/invalid (likely an unresolved Git LFS pointer) and no download URL is configured."

    try:
        with st.spinner(f"Downloading {cfg['label']} weights ({cfg['expected_size']/1e6:.0f} MB, first run only)..."):
        ...
        return None, f"Failed to download {cfg['file']} from {url}: {e}"

    if not _is_valid_model_file(path, cfg["expected_size"]):
        return None, f"Downloaded {cfg['file']} but it still looks invalid (size {os.path.getsize(path)} bytes) -- check the URL is a direct binary link, not an LFS pointer or HTML page."
    return path, None

@st.cache_resource(show_spinner=False)
def load_models():
    """
    Loads every .h5 model, downloading real weights first if the local copy
    is missing or is an unresolved Git LFS pointer stub.
...
def predict_with_heuristic(img):
    ...
    # 3. Texture analysis    texture = np.var(img_gray)
