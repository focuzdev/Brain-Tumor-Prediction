...
if clicked and img:
    ...
    # ===========================================================
    # PREDICTION CARD in right column
    # ===========================================================
    with col_out:
        st.markdown(f"""
<div class="pred-card">
  ...
  <div class="risk-chip {rc}">
    <span class="rdot {dc}"></span>{rl} RISK
        ...
        st.markdown('<div class="slbl" style="margin-top:12px;">Class Probability Distribution</div>',
                    unsafe_allow_html=True)
        fp = prob_fig(preds, pidx)
        st.pyplot(fp, use_container_width=True); plt.close()

    # ===========================================================
    # GRAD-CAM HEATMAP SECTION  (FULL WIDTH - always shown)
    # ===========================================================
    st.markdown("---")
...
<div class="hm-section">
  ...
  <div class="hm-header">
    <div class="hm-hdr-left">
      <div class="hm-title">Grad-CAM Tumor Region Heatmap</div>
      <div class="hm-sub">
        Gradient-Weighted Class Activation Mapping &nbsp;|&nbsp; {note} &nbsp;|&nbsp;
        Pure CNN &nbsp;|&nbsp; No VLM / LLaMA Required
      </div>
    </div>
    <div class="hm-legend">
      <div class="hm-leg">
        <span class="hm-swatch"
          style="background:linear-gradient(90deg,#00007f,#0000ff,#007fff,#00ffff)"></span>
        Low attention
      </div>
      <div class="hm-leg">
        <span class="hm-swatch"
          style="background:linear-gradient(90deg,#ffff00,#ff7f00,#ff0000,#7f0000)"></span>
        High attention / Tumor region
      </div>
    </div>
  </div>
  ...
  <div class="hm-stats">
    <div class="hm-stat">
      <div class="hm-sv">{mean_a:.3f}</div>
      <div class="hm-sl">Mean Activation</div>
    </div>
    <div class="hm-stat">
      <div class="hm-sv">{p90_a:.3f}</div>
      <div class="hm-sl">90th Percentile</div>
    </div>
    <div class="hm-stat">
      <div class="hm-sv">{focus_p:.1f}%</div>
      <div class="hm-sl">High-Activation Area</div>
    </div>
    <div class="hm-stat">
      <div class="hm-sv">{max_a:.3f}</div>
...
""", unsafe_allow_html=True)
    ...
    with c1:
        ...
        st.markdown('</div>', unsafe_allow_html=True)
        st.markdown('<div class="hm-col-note">Raw input scan<br>before preprocessing</div>',
                    unsafe_allow_html=True)

    with c2:
        st.markdown('<div class="hm-col-lbl">Grad-CAM Overlay</div>', unsafe_allow_html=True)
        st.markdown('<div class="hm-img-frame">', unsafe_allow_html=True)
        st.image(overlay_img, use_column_width=True)
        st.markdown('</div>', unsafe_allow_html=True)
        st.markdown(
            f'<div class="hm-col-note">Red/yellow = tumor region<br>'
            f'CNN attention for <strong style="color:#38bdf8">{pcls}</strong></div>',
            unsafe_allow_html=True)
    ...
    with c3:
        ...
        st.markdown('<div class="hm-col-note">Normalised activation<br>from last conv layer</div>',
                    unsafe_allow_html=True)
    ...
    with c4:
        ...
        st.markdown('<div class="hm-col-note">Bars colored by jet<br>activation intensity</div>',
                    unsafe_allow_html=True)

    # Jet scale + explainer + close hm-section
    st.markdown(f"""
  </div>
    ...
    <div class="hm-exp-grid">
      <div class="hm-exp-item">
        ...
        <div class="hm-exp-b">The 224x224 MRI passes through ResNet50V2.
          The last conv layer outputs a 7x7 spatial feature map with 2048 channels,
          each detecting different visual patterns at specific locations.</div>
      </div>
      <div class="hm-exp-item">
        <div class="hm-exp-t">Step 2 - Gradient Backprop</div>
        <div class="hm-exp-b">TensorFlow GradientTape measures how strongly each
          feature map channel contributed to the "{pcls}" score.
          Channels that most raised the score get the highest weight.</div>
      </div>
      <div class="hm-exp-item">
        <div class="hm-exp-t">Step 3 - Weighted Sum + ReLU</div>
        <div class="hm-exp-b">Each feature map is multiplied by its importance weight,
          summed, and ReLU-activated. The coarse 7x7 result is upscaled to 224x224
          via bilinear interpolation for display.</div>
      </div>
      <div class="hm-exp-item">
        <div class="hm-exp-t">Scan Statistics</div>
        <div class="hm-exp-b">Mean: <strong style="color:#38bdf8">{mean_a:.3f}</strong>,
          ...
          {focus_p:.1f}% of pixels exceed 0.5 activation -
