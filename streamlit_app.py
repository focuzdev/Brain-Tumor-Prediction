...
def mobilenet_sanity_check():
    """
    A model that predicts the SAME class for every input regardless of
    content — exactly what was reported after MobileNetV2 was added to
    this app — is a textbook symptom of one of a small number of causes:

      1. CLASS INDEX MISMATCH (by far the most common cause of this exact
         symptom). If MobileNetV2 was trained with a different
         class-to-index mapping than CLASS_NAMES = [Glioma, Meningioma,
         No Tumor, Pituitary Tumor] used everywhere in this app — e.g. its
         training folders were read in a different order, or a different
         script/notebook built its label list independently from
         ResNet50V2's — then averaging its raw output vector with
         ResNet50V2's is not combining two opinions about the same 4
         classes, it's combining two DIFFERENT permutations of the same 4
         numbers. Action: pull up whatever generated mobilenetv2_final.h5
         and check its class_indices (Keras ImageDataGenerator /
         image_dataset_from_directory print this, or save it explicitly
         next time) against CLASS_NAMES order here.

      2. DEGENERATE LOAD. keras.models.load_model() can "succeed" (no
         exception) while silently failing to restore custom layers,
         producing near-random or near-constant output. Action: load the
         model standalone and confirm its raw prediction on a few very
         different images actually differs.

      3. A BAD CHECKPOINT. This app's own training run showed a visibly
         unstable MobileNetV2 validation-accuracy curve (large dips at
         the unfreeze points). If mobilenetv2_final.h5 is the LAST epoch
         rather than the BEST epoch, it may have been saved mid-collapse.

      This function does not try to guess which of the three it is — it
      just refuses to trust MobileNetV2 in the live ensemble unless it can
      prove, right now, against the bundled reference images, that it
      actually produces different top-1 predictions for genuinely
      different classes. If it can't, the ensemble silently falls back to
      solo ResNet50V2 (which was working before MobileNetV2 was added)
      instead of quietly blending in a broken model's output.
    """
    mob = load_mobilenet()
    if mob is None:
        return False, "MobileNetV2 not loaded — nothing to check.", {}
    if not os.path.isdir(SAMPLE_DIR):
        return False, ("No samples/ folder found to sanity-check MobileNetV2 against. "
                        "Running an unverified second model in a medical-decision-support "
                        "tool is not acceptable, so it is disabled until it can be verified."), {}
    ...
    for cls_name, fname in SAMPLES.items():
        ...
        try:
            ...
            arr = preprocess_for_mobilenet(im)
            p   = mob.predict(arr, verbose=0)[0]
            results[cls_name] = {
                "predicted":   CLASS_NAMES[int(np.argmax(p))],
                "confidence":  round(float(np.max(p)) * 100, 1),
                "full_probs":  [round(float(x), 4) for x in p],
            }
        except Exception as e:
            return False, f"MobileNetV2 threw an error on reference image '{fname}': {e}", results

    if len(results) < 3:
        return False, ("Fewer than 3 reference images found in samples/ — not enough to "
                        "safely sanity-check MobileNetV2. Disabled until more reference "
                        "images are available."), results

    predicted_classes = set(r["predicted"] for r in results.values())
    if len(predicted_classes) == 1:
        only = list(predicted_classes)[0]
        detail = (
            f"MobileNetV2 predicted '{only}' for every single reference image, "
            f"including ones whose true class is NOT {only}. It is not "
            f"discriminating between inputs at all and has been automatically "
            f"excluded from the ensemble. This is a training/export bug (see the "
            f"three likely causes above) — it needs to be fixed at the source, "
            f"not worked around here."
        )
        return False, detail, results

    return True, "MobileNetV2 correctly distinguished between reference classes.", results
...
def _resize_and_clean(img):
    """
    Resize PIL image to (224,224) and return float32 array in [0, 255].
    Backbone-specific scaling (resnet_preprocess / mobilenet_preprocess)
    is applied AFTER this, separately per model.
    """
    # Guarantee RGB mode — handles greyscale, RGBA, palette images
    if img.mode != "RGB":
        img = img.convert("RGB")
    resized = img.resize(IMG_SIZE, Image.LANCZOS)
    # Keep values in [0, 255] float32 — resnet_preprocess expects this
    arr = np.array(resized, dtype=np.float32)
    # Safety: ensure shape is exactly (224, 224, 3)
    if arr.ndim == 2:
        arr = np.stack([arr, arr, arr], axis=-1)
    if arr.shape[-1] == 4:
        arr = arr[:, :, :3]
    return arr[:IMG_SIZE[0], :IMG_SIZE[1], :3]
...
with st.sidebar:
    ...
    with st.expander("Advanced"):
        temperature = st.slider(
            "Calibration Temperature", 1.0, 2.5, 1.0, 0.1,
            help="T=1.0 uses raw model output (recommended). "
                 "Increasing T softens confidence scores. "
                 "Only change if you have a reliability diagram showing miscalibration."
        )
    ...
    _mob_sane, _mob_detail, _mob_ref_results = (False, "", {})
    if _mob_present or _mob_downloadable:
        # This actually runs MobileNetV2 against the bundled reference
        # images right now, cached for the session — it's the same check
        # used to gate the live ensemble below, surfaced here so it's
        # impossible to miss.
        try:
            _mob_sane, _mob_detail, _mob_ref_results = mobilenet_sanity_check()
        except Exception as _e:
            _mob_sane, _mob_detail = False, f"Sanity check crashed: {_e}"

    if _mob_present and _mob_sane:
        _ensemble_line = "+MobileNetV2 (loaded, passed sanity check)"
    elif _mob_present and not _mob_sane:
        _ensemble_line = "MobileNetV2 DISABLED — failed sanity check"
    ...
    st.code(f"Model    : ResNet50V2\nEnsemble : {_ensemble_line}\n"
            ...
            "Input    : 224x224 RGB\nXAI      : Grad-CAM\n"
            "Accuracy : 95.31% (ensemble)", language="text")

    if _mob_present and not _mob_sane:
        st.error(f"🚫 MobileNetV2 excluded from predictions: {_mob_detail}")
        if _mob_ref_results:
            st.caption("Per-reference-image MobileNetV2 output (this is your diagnostic evidence):")
            st.table({
                "True class":       list(_mob_ref_results.keys()),
                "MobileNetV2 said": [v["predicted"] for v in _mob_ref_results.values()],
                "Confidence %":     [v["confidence"] for v in _mob_ref_results.values()],
            })
            st.caption(
                "If every row says the same class, this is almost certainly a "
                "class-index mismatch between how MobileNetV2 was trained/exported "
                "and CLASS_NAMES order in this app, a bad (non-best) checkpoint, or "
                "a load that silently produced degenerate weights. Predictions "
                "below are running on ResNet50V2 alone until this is fixed at the "
                "source and passes this check."
            )
    elif not (_mob_present or _mob_downloadable):
        st.caption(
...
[data-testid="stSelectbox"] svg {{ fill:{_sb_ico} !important; }}
    ...
    img = src = None
    if uploaded:
        # Simple, direct load — matches the original working pipeline.
        # PIL's Image.open() on a Streamlit UploadedFile is reliable;
        # the buffer stays valid for the duration of the request.
        # We call .load() to force pixel decode, then .convert("RGB")
        # to guarantee 3-channel uint8 input to the CNN.
        try:
            img = Image.open(uploaded)
            img.load()                   # force full pixel decode
            img = img.convert("RGB")     # ensure 3-channel, drop alpha/palette
            img = img.copy()             # detach from upload buffer
            src = "upload"
        except Exception as _e:
...
""", unsafe_allow_html=True)
    ...
    with st.spinner("Running CNN inference..."):
        ...
        if model:
            # ── ResNet50V2 inference ──────────────────────────────
            raw_resnet = model.predict(arr, verbose=0)[0]  # shape: (4,)

            # ── MobileNetV2 ensemble (soft-vote) ─────────────────
            mob_model                = load_mobilenet()
            mob_sane, mob_detail, _  = (
                mobilenet_sanity_check()
                if mob_model is not None
                else (False, "Not loaded.", {})
            )
            if mob_model is not None and mob_sane:
                try:
                    ...
                    raw           = raw_resnet
                    ensemble_mode = f"ResNet50V2 (MobileNetV2 failed: {_ee})"
            elif mob_model is not None and not mob_sane:
                raw           = raw_resnet
                ensemble_mode = f"ResNet50V2 (MobileNetV2 disabled: {mob_detail[:60]})"
            else:
                raw           = raw_resnet
                ensemble_mode = "ResNet50V2 single model"

            # ── Temperature scaling ──────────────────────────────
            # Applied ONLY when T != 1.0. At T=1.0 (default) the raw
            # softmax output is used directly — no distortion.
            T = temperature
            if abs(T - 1.0) > 0.05:
                ...
                preds = raw / raw.sum()  # renormalise for numerical safety
            ...
            # No model loaded — demo predictions for sample images only.
            # For uploaded external images, do not fabricate a result.
            if src == "upload":
                st.error(
                    "**No model loaded.** The CNN model file (`brain_tumor_model.h5`) "
                    "was not found. Upload predictions require the real model. "
                    "Please check Streamlit Cloud settings or deploy with the model file."
                )
                st.stop()
            else:
    ...
    _glioma_men_ambiguous = {pcls, _second_cls} == {"Glioma", "Meningioma"} and is_close_call
