...
.topnav{{
  ...
  background:{"rgba(4,9,15,.95)" if _dk else "rgba(255,255,255,.97)"};
...
.nav-logo{{width:34px;height:34px;border-radius:9px;font-size:16px;background:linear-gradient(135deg,#1e40af,#0e7490);display:flex;align-items:center;justify-content:center;box-shadow:0 0 16px rgba(56,189,248,.40);flex-shrink:0}}
.nav-name{{font-family:'Space Grotesk',sans-serif;font-size:15px;font-weight:700;color:{"#e2e8f0" if _dk else "#0a1628"};letter-spacing:-.3px;white-space:nowrap}}
.nav-name span{{color:#38bdf8}}
.nav-tagline{{font-family:'DM Mono',monospace;font-size:7.5px;color:{"rgba(255,255,255,.45)" if _dk else "#4a6580"};letter-spacing:.13em;text-transform:uppercase;margin-top:1px;white-space:nowrap}}
.nav-right{{display:flex;gap:5px;align-items:center;flex-wrap:wrap;justify-content:flex-end}}
.chip{{font-family:'DM Mono',monospace;font-size:8.5px;font-weight:500;padding:3px 9px;border-radius:20px;letter-spacing:.05em;text-transform:uppercase;white-space:nowrap}}
...
/* hide chips on very small screens, keep logo + name + toggle */
@media(max-width:600px){{
  .chip{{display:none}}
  .nav-tagline{{display:none}}
  .topnav{{padding:.55rem .9rem}}
}}
...
.hero{{
  ...
  border-bottom:1px solid {"rgba(56,189,248,.09)" if _dk else "#a8cfe6"};
}}
...
.hero-top{{display:flex;align-items:flex-start;justify-content:space-between;flex-wrap:wrap;gap:1.2rem}}
.hero-h1{{font-family:'Space Grotesk',sans-serif;font-size:clamp(1.5rem,4vw,2.55rem);font-weight:700;color:{"#e2e8f0" if _dk else "#0a1628"};letter-spacing:-.5px;line-height:1.15;margin-bottom:.45rem}}
...
.hero-desc{{font-size:clamp(12px,1.8vw,13.5px);color:{"rgba(255,255,255,.70)" if _dk else "#2d4a6b"};line-height:1.72;max-width:520px}}
.hero-stats{{display:flex;gap:1.2rem;flex-wrap:wrap;align-items:flex-start}}
.hs{{text-align:right}}
.hs-n{{font-family:'Space Grotesk',sans-serif;font-size:clamp(18px,3vw,27px);font-weight:700;background:linear-gradient(92deg,#38bdf8,#818cf8);-webkit-background-clip:text;-webkit-text-fill-color:transparent;background-clip:text;line-height:1}}
.hs-l{{font-family:'DM Mono',monospace;font-size:8px;color:{"rgba(255,255,255,.55)" if _dk else "#2d4a6b"};text-transform:uppercase;letter-spacing:.1em;margin-top:3px}}
.hero-div{{height:1px;margin:1.2rem 0;background:linear-gradient(90deg,rgba(56,189,248,.30),rgba(129,140,248,.18),transparent)}}

/* pipeline — wraps to 2 rows on mobile */
...
.pip-num{{width:26px;height:26px;border-radius:50%;background:linear-gradient(135deg,#1d4ed8,#0891b2);font-family:'DM Mono',monospace;font-size:10px;font-weight:600;color:#fff;display:flex;align-items:center;justify-content:center;box-shadow:0 0 12px rgba(56,189,248,.32);flex-shrink:0}}
.pip-txt{{font-size:11px;color:{"rgba(255,255,255,.65)" if _dk else "#2d4a6b"};line-height:1.35}}
.pip-txt strong{{color:{"rgba(255,255,255,.90)" if _dk else "#0a1628"};display:block;font-size:10.5px}}
.pip-arr{{color:{"rgba(56,189,248,.40)" if _dk else "#4a8fa8"};font-size:18px;padding:0 6px}}
@media(max-width:600px){{
  .pip-arr{{padding:0 4px;font-size:14px}}
  .hero-stats{{gap:.9rem}}
  .hero{{padding:1.6rem .9rem 1.6rem}}
}}

/* ══ WRAP + GLASS ══ */
.wrap{{max-width:1400px;margin:0 auto;padding:1.5rem 1rem 4rem}}
@media(min-width:768px){{.wrap{{padding:2rem 1.6rem 5rem}}}}
@media(min-width:1200px){{.wrap{{padding:2rem 2.2rem 5rem}}}}
.glass{{background:{"rgba(255,255,255,.030)" if _dk else "rgba(255,255,255,.92)"};border:1px solid {"rgba(255,255,255,.075)" if _dk else "#b8d4e8"};border-radius:16px;padding:1.2rem;backdrop-filter:blur(12px);box-shadow:0 8px 40px {"rgba(0,0,0,.35)" if _dk else "rgba(10,22,80,.08)"}}}
@media(min-width:768px){{.glass{{border-radius:20px;padding:1.6rem}}}}
.slbl{{font-family:'DM Mono',monospace;font-size:9px;color:{"rgba(56,189,248,.80)" if _dk else "#0e6b8a"};text-transform:uppercase;letter-spacing:.15em;margin-bottom:10px;display:flex;align-items:center;gap:8px}}
.slbl::after{{content:'';flex:1;height:1px;background:{"rgba(56,189,248,.20)" if _dk else "#9ecadb"}}}

/* ══ PREDICTION CARD ══ */
.pred-card{{background:linear-gradient(135deg,{"rgba(14,30,70,.82)" if _dk else "#daeeff"},{"rgba(8,20,48,.92)" if _dk else "#cce5f8"});border:1px solid {"rgba(56,189,248,.22)" if _dk else "#7ab8d4"};border-radius:14px;padding:1.2rem 1.3rem;margin-bottom:12px;position:relative;overflow:hidden;box-shadow:0 8px 32px {"rgba(0,0,0,.25)" if _dk else "rgba(10,22,80,.10)"}}}
...
.pred-eyebrow{{font-family:'DM Mono',monospace;font-size:8.5px;color:{"rgba(56,189,248,.80)" if _dk else "#0e6b8a"};text-transform:uppercase;letter-spacing:.16em;margin-bottom:6px}}
.pred-name{{font-family:'Space Grotesk',sans-serif;font-size:clamp(24px,5vw,38px);font-weight:700;color:{"#f8fafc" if _dk else "#0a1628"};letter-spacing:-1px;line-height:1.04;margin-bottom:12px}}
...
.conf-v{{font-family:'Space Grotesk',sans-serif;font-size:13px;color:#38bdf8;font-weight:600}}
.conf-track{{background:{"rgba(255,255,255,.10)" if _dk else "#b8d8ec"};border-radius:8px;height:5px;overflow:hidden;margin-bottom:12px}}
.conf-fill{{height:100%;border-radius:8px;background:linear-gradient(90deg,#1d4ed8,#0891b2,#38bdf8)}}
.risk-chip{{display:inline-flex;align-items:center;gap:6px;padding:5px 12px;border-radius:20px;font-family:'DM Mono',monospace;font-size:9.5px;font-weight:500;letter-spacing:.07em;text-transform:uppercase}}
...
/* ══ HEATMAP SECTION ══ */
.hm-section{{background:{"rgba(2,6,14,.97)" if _dk else "#f4faff"};border:1px solid {"rgba(56,189,248,.20)" if _dk else "#a0cce0"};border-radius:16px;overflow:hidden;box-shadow:0 8px 32px {"rgba(0,0,0,.45)" if _dk else "rgba(10,22,80,.08)"};margin:1.4rem 0}}
@media(min-width:768px){{.hm-section{{border-radius:22px;margin:1.8rem 0}}}}
.hm-header{{background:{"rgba(4,10,24,1)" if _dk else "#daeeff"};border-bottom:1px solid {"rgba(56,189,248,.12)" if _dk else "#a8cfe0"};padding:.9rem 1rem;display:flex;align-items:flex-start;justify-content:space-between;flex-wrap:wrap;gap:8px}}
@media(min-width:768px){{.hm-header{{padding:1.1rem 1.7rem}}}}
.hm-title{{font-family:'Space Grotesk',sans-serif;font-size:clamp(13px,2vw,16px);font-weight:600;color:{"#e2e8f0" if _dk else "#0a1628"};letter-spacing:-.2px}}
.hm-sub{{font-family:'DM Mono',monospace;font-size:8px;color:{"rgba(255,255,255,.45)" if _dk else "#2d4a6b"};text-transform:uppercase;letter-spacing:.1em;margin-top:3px;line-height:1.6}}
.hm-legend{{display:flex;align-items:center;gap:10px;flex-wrap:wrap}}
.hm-leg{{display:flex;align-items:center;gap:5px;font-family:'DM Mono',monospace;font-size:8.5px;color:{"rgba(255,255,255,.60)" if _dk else "#1a3550"}}}
.hm-swatch{{width:24px;height:8px;border-radius:3px}}
.hm-stats{{display:grid;grid-template-columns:repeat(2,1fr);gap:0;border-bottom:1px solid {"rgba(255,255,255,.08)" if _dk else "#b8d8ec"}}}
@media(min-width:600px){{.hm-stats{{grid-template-columns:repeat(4,1fr)}}}}
.hm-stat{{padding:10px 12px;border-right:1px solid {"rgba(255,255,255,.08)" if _dk else "#b8d8ec"};border-bottom:1px solid {"rgba(255,255,255,.08)" if _dk else "#b8d8ec"};text-align:center}}
@media(min-width:600px){{.hm-stat{{border-bottom:none;padding:13px 16px}}}}
.hm-stat:last-child{{border-right:none}}
.hm-sv{{font-family:'Space Grotesk',sans-serif;font-size:18px;font-weight:600;color:#38bdf8;line-height:1}}
.hm-sl{{font-family:'DM Mono',monospace;font-size:8px;color:{"rgba(255,255,255,.45)" if _dk else "#2d4a6b"};text-transform:uppercase;letter-spacing:.08em;margin-top:3px}}
.hm-grid{{padding:1rem}}
@media(min-width:768px){{.hm-grid{{padding:1.4rem 1.6rem}}}}
.hm-col-lbl{{font-family:'DM Mono',monospace;font-size:8.5px;color:{"rgba(255,255,255,.55)" if _dk else "#1a3550"};text-transform:uppercase;letter-spacing:.1em;text-align:center;margin-bottom:6px}}
.hm-col-note{{font-family:'DM Mono',monospace;font-size:8.5px;color:{"rgba(255,255,255,.45)" if _dk else "#2d4a6b"};text-align:center;margin-top:6px;line-height:1.6}}
.hm-img-frame{{border-radius:10px;overflow:hidden;border:1px solid {"rgba(255,255,255,.10)" if _dk else "#a8cfe0"};box-shadow:0 4px 16px {"rgba(0,0,0,.40)" if _dk else "rgba(10,22,80,.10)"}}}
.cscale{{background:{"rgba(255,255,255,.025)" if _dk else "#ffffff"};margin:0 1rem 1rem;border-radius:8px;padding:8px 12px;border:1px solid {"rgba(255,255,255,.08)" if _dk else "#b8d8ec"}}}
@media(min-width:768px){{.cscale{{margin:0 1.6rem 1.4rem}}}}
.cscale-bar{{height:10px;border-radius:3px;background:linear-gradient(90deg,#00007f 0%,#0000ff 12%,#007fff 24%,#00ffff 36%,#7fff7f 50%,#ffff00 64%,#ff7f00 76%,#ff0000 88%,#7f0000 100%);margin-bottom:4px}}
.cscale-lbls{{display:flex;justify-content:space-between;font-family:'DM Mono',monospace;font-size:8px;color:{"rgba(255,255,255,.50)" if _dk else "#2d4a6b"}}}
.hm-explain{{background:{"rgba(255,255,255,.025)" if _dk else "#ffffff"};border-top:1px solid {"rgba(255,255,255,.08)" if _dk else "#a8cfe0"};padding:1rem}}
@media(min-width:768px){{.hm-explain{{padding:1.3rem 1.7rem}}}}
.hm-exp-title{{font-family:'DM Mono',monospace;font-size:8.5px;color:{"rgba(56,189,248,.80)" if _dk else "#0e6b8a"};text-transform:uppercase;letter-spacing:.13em;margin-bottom:10px}}
.hm-exp-grid{{display:grid;grid-template-columns:1fr;gap:8px}}
@media(min-width:480px){{.hm-exp-grid{{grid-template-columns:repeat(2,1fr)}}}}
@media(min-width:900px){{.hm-exp-grid{{grid-template-columns:repeat(auto-fit,minmax(190px,1fr))}}}}
.hm-exp-item{{background:{"rgba(255,255,255,.025)" if _dk else "#ffffff"};border:1px solid {"rgba(255,255,255,.08)" if _dk else "#a8cfe0"};border-radius:10px;padding:10px 12px}}
.hm-exp-t{{font-family:'DM Mono',monospace;font-size:8.5px;color:#38bdf8;margin-bottom:4px;font-weight:500}}
.hm-exp-b{{font-size:11px;color:{"rgba(255,255,255,.80)" if _dk else "#0a1628"};line-height:1.62}}

/* ══ REPORT BLOCKS ══ */
.rb    {{border-left:3px solid rgba(147,197,253,.55);background:{"rgba(255,255,255,.028)" if _dk else "#ffffff"};border-radius:0 10px 10px 0;padding:12px 14px;margin-bottom:10px}}
...
.rb-t{{font-family:'DM Mono',monospace;font-size:8.5px;color:{"rgba(255,255,255,.55)" if _dk else "#2d4a6b"};text-transform:uppercase;letter-spacing:.12em;margin-bottom:6px}}
.rb-b{{font-size:12.5px;line-height:1.85;color:{"rgba(255,255,255,.88)" if _dk else "#0a1628"}}}

/* ══ DISCLAIMER ══ */
.disc{{background:{"rgba(245,158,11,.08)" if _dk else "#fef6d8"};border:1px solid {"rgba(245,158,11,.30)" if _dk else "#d4a017"};border-left:3px solid rgba(245,158,11,.80);border-radius:0 10px 10px 0;padding:12px 14px;font-family:'DM Mono',monospace;font-size:9.5px;color:{"#fcd34d" if _dk else "#7a4800"};line-height:1.78;margin-top:18px}}
...
.stTabs [data-baseweb="tab-list"]{{background:{"rgba(255,255,255,.050)" if _dk else "#ddeef8"} !important;border-radius:10px !important;padding:3px !important;gap:2px !important;border:1px solid {"rgba(255,255,255,.090)" if _dk else "#a8cfe0"} !important}}
.stTabs [data-baseweb="tab"]{{border-radius:7px !important;font-family:'DM Mono',monospace !important;font-size:10px !important;color:{"rgba(255,255,255,.60)" if _dk else "#1e3a55"} !important;padding:7px 12px !important}}
.stTabs [aria-selected="true"]{{background:rgba(56,189,248,.15) !important;color:#38bdf8 !important;box-shadow:none !important}}
[data-testid="stFileUploader"]{{border:2px dashed {"rgba(56,189,248,.65)" if _dk else "#0e7490"} !important;border-radius:14px !important;background:{"rgba(20,80,130,.28)" if _dk else "#d8eefb"} !important}}
...
[data-testid="stFileUploader"] button,[data-testid="stFileUploaderDropzone"] button{{background:linear-gradient(135deg,#1e40af,#0e7490) !important;color:#fff !important;border:none !important;border-radius:10px !important;font-family:'Space Grotesk',sans-serif !important;font-weight:600 !important;font-size:13px !important;padding:10px 20px !important;opacity:1 !important}}
.stButton > button{{background:linear-gradient(135deg,#1e3a8a 0%,#0e7490 55%,#0891b2 100%) !important;color:#fff !important;border:none !important;border-radius:12px !important;font-family:'Space Grotesk',sans-serif !important;font-weight:600 !important;font-size:14px !important;padding:12px 24px !important;width:100% !important;transition:all .22s ease !important}}
.stButton > button:hover{{transform:translateY(-2px) !important;box-shadow:0 8px 32px rgba(14,116,144,.55) !important}}
[data-testid="stSelectbox"] > div > div{{background:{"rgba(255,255,255,.055)" if _dk else "#ffffff"} !important;border:1px solid {"rgba(255,255,255,.12)" if _dk else "#7ab8d0"} !important;border-radius:10px !important;color:{"#e2e8f0" if _dk else "#0a1628"} !important}}
[data-testid="stMetric"]{{background:{"rgba(255,255,255,.055)" if _dk else "#ffffff"} !important;border:1px solid {"rgba(255,255,255,.10)" if _dk else "#a8cfe0"} !important;border-radius:12px !important;padding:11px 14px !important}}
[data-testid="stMetricLabel"]{{color:{"rgba(255,255,255,.55)" if _dk else "#2d4a6b"} !important;font-size:11px !important}}
[data-testid="stMetricValue"]{{font-family:'Space Grotesk',sans-serif !important;color:#38bdf8 !important;font-size:20px !important}}
...
[data-testid="stDownloadButton"] > button{{background:{"rgba(56,189,248,.10)" if _dk else "#daeeff"} !important;border:1px solid {"rgba(56,189,248,.28)" if _dk else "#4a9ab8"} !important;color:{"#38bdf8" if _dk else "#084e65"} !important;font-family:'DM Mono',monospace !important;font-size:11px !important;border-radius:9px !important;padding:9px 14px !important;width:100% !important;box-shadow:none !important}}
...
hr{{border-color:{"rgba(255,255,255,.10)" if _dk else "#c2d8e8"} !important;margin:1.4rem 0 !important}}
[data-testid="stImage"] img{{border-radius:10px !important;border:1px solid {"rgba(255,255,255,.10)" if _dk else "#a8cfe0"} !important;display:block !important;width:100% !important;height:auto !important}}
...
/* theme toggle */
.theme-toggle{{width:32px;height:32px;border-radius:50%;background:{"rgba(28,38,68,.85)" if _dk else "#daeeff"};border:1px solid {"rgba(56,189,248,.50)" if _dk else "#4a9ab8"};color:{"#7dd3fc" if _dk else "#084e65"};font-size:15px;line-height:1;display:inline-flex;align-items:center;justify-content:center;cursor:pointer;flex-shrink:0;box-shadow:0 2px 8px rgba(0,0,0,.15);-webkit-appearance:none;appearance:none;outline:none;text-decoration:none;user-select:none;transition:background .18s,transform .18s !important}}
.theme-toggle:hover{{background:{"rgba(56,189,248,.25)" if _dk else "#b8dff0"};transform:scale(1.12) rotate(14deg)}}

/* ══ RESPONSIVE COLUMN STACKING ══ */
/* On mobile Streamlit columns stack vertically by default — ensure
   our custom HTML sections also stack and images stay full-width */
@media(max-width:768px){{
  .hm-grid [class*="col"],.hm-grid > div{{width:100% !important}}
  .pred-name{{font-size:clamp(22px,7vw,34px)}}
...
with col_in:
    ...
    st.markdown("""<p style="font-family:'DM Mono',monospace;font-size:9px;
      color:rgba(255,255,255,.55);text-align:center;padding:4px 0 10px;
    ...
    if uploaded:
        ...
        st.markdown("""
<div style="border:1.5px dashed rgba(56,189,248,.25);border-radius:12px;
  padding:2.5rem;text-align:center;background:rgba(56,189,248,.03);margin:8px 0">
  <div style="font-size:34px;margin-bottom:10px;opacity:.4">🩻</div>
  <div style="font-family:'DM Mono',monospace;font-size:9.5px;
    color:rgba(255,255,255,.45);letter-spacing:.09em;line-height:1.8;">
