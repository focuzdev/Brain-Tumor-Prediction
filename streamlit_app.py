...
===========================================================
UPGRADED: Enhanced Grad-CAM + Better Claude Integration
(Keeping all working functionality)
"""
...
# ──────────────────────────────────────────────────────────────────────────────
# CSS — Light Clinical Theme (YOUR ORIGINAL WORKING CSS)
# ──────────────────────────────────────────────────────────────────────────────
st.markdown("""
<style>
...
# ──────────────────────────────────────────────────────────────────────────────
# Claude AI Report (KEEP YOUR ORIGINAL WORKING VERSION)
# ──────────────────────────────────────────────────────────────────────────────
...
Be conservative, evidence-based, and never hallucinate findings."""
        ...
        response = client.messages.create(
            model="claude-3-sonnet-20241022",  # FIXED: Use correct model name
            max_tokens=1500,
...
if analyze_clicked and pil_image:
    ...
    # ── Grad-CAM Display (Enhanced with better visualization) ──
    if show_gradcam and gradcam_overlay:
        st.markdown("---")
        st.markdown('<div class="section-label">Explainability · Grad-CAM Visualization</div>',
                    unsafe_allow_html=True)
        
        # Create 2-column layout for better comparison
        gc1, gc2 = st.columns(2)
        with gc1:
            st.image(pil_image, caption="Original MRI", use_column_width=True)
        with gc2:
            st.image(gradcam_overlay, caption="Grad-CAM Overlay (Model Attention)", use_column_width=True)
        ...
        if heatmap is not None:
            ...
            ax2.set_title("Model Attention Heatmap", fontsize=14, color="#1a2d50")
            ax2.axis("off")
            ax2.set_facecolor("#f8faff")
            fig2.patch.set_facecolor("#f8faff")
            cbar = fig2.colorbar(im, ax=ax2, fraction=0.046, pad=0.04)
            cbar.set_label("Activation Intensity", color="#7a96bb")
            cbar.ax.tick_params(colors="#7a96bb")
            ...
            if max_activation > 0.7:
                st.success("✅ **Grad-CAM Interpretation:** Strong focal activation detected - model is focusing on specific region consistent with tumor localization.")
            elif max_activation > 0.4:
                st.info("📊 **Grad-CAM Interpretation:** Moderate activation pattern - model shows reasonable attention to relevant regions.")
            else:
                st.warning("⚠️ **Grad-CAM Interpretation:** Low activation detected - model attention is diffuse, suggesting uncertainty in localization.")
    ...
    st.download_button(
        ...
        data=json.dumps({
            ...
            "gradcam_analysis": {
                "max_activation": float(heatmap.max()) if heatmap is not None else None,
                "mean_activation": float(heatmap.mean()) if heatmap is not None else None
            } if heatmap is not None else {},
            **report,
        }, indent=2),
