...
# PAGE CONFIG
# ================================================================
# Theme must be initialised before set_page_config so the first render is correct
...
.theme-dark {
  --bg-base:        #04090f;
  --bg-grad-a:      rgba(14,58,150,.45);
  --bg-grad-b:      rgba(8,100,160,.25);
  --bg-grad-c:      rgba(5,20,50,.4);
  --text-primary:   #e2e8f0;
  --text-muted:     rgba(255,255,255,.42);
  --text-dim:       rgba(255,255,255,.22);
  --nav-bg:         rgba(4,9,15,.88);
  --nav-border:     rgba(56,189,248,.12);
  --hero-bg-a:      #040b1a;
  --hero-bg-b:      #07142e;
  --hero-bg-c:      #040c1e;
  --hero-border:    rgba(56,189,248,.09);
  --glass-bg:       rgba(255,255,255,.028);
  --glass-border:   rgba(255,255,255,.07);
  --slbl-color:     rgba(56,189,248,.55);
  --slbl-line:      rgba(56,189,248,.12);
  --upload-bg:      rgba(20,80,130,.22);
  --upload-border:  rgba(56,189,248,.55);
  --upload-text:    #7dd3fc;
  --upload-btn-bg:  linear-gradient(135deg,#1e40af,#0e7490);
  --upload-btn-txt: #ffffff;
  --rb-bg:          rgba(255,255,255,.025);
  --disc-bg:        rgba(245,158,11,.05);
  --disc-border:    rgba(245,158,11,.18);
  --disc-text:      rgba(253,211,77,.65);
  --hr-color:       rgba(255,255,255,.06);
  --tab-bg:         rgba(255,255,255,.035);
  --tab-border:     rgba(255,255,255,.06);
  --tab-color:      rgba(255,255,255,.38);
  --metric-bg:      rgba(255,255,255,.04);
  --metric-border:  rgba(255,255,255,.07);
  --metric-lbl:     rgba(255,255,255,.38);
  --toggle-bg:      rgba(56,189,248,.10);
  --toggle-border:  rgba(56,189,248,.30);
  --toggle-text:    #38bdf8;
  --toggle-icon:    "🌙";
  --toggle-label:   "Dark Mode";
  --hm-bg:          rgba(2,6,14,.95);
  --hm-border:      rgba(56,189,248,.14);
  --hm-header:      rgba(4,10,24,1);
  --pred-bg-a:      rgba(14,30,70,.8);
  --pred-bg-b:      rgba(8,20,48,.9);
}

.theme-light {
  --bg-base:        #f0f4f8;
  --bg-grad-a:      rgba(186,220,255,.55);
  --bg-grad-b:      rgba(180,230,245,.35);
  --bg-grad-c:      rgba(210,230,250,.3);
  --text-primary:   #0f1e36;
  --text-muted:     rgba(15,30,54,.60);
  --text-dim:       rgba(15,30,54,.42);
  --nav-bg:         rgba(240,247,255,.95);
  --nav-border:     rgba(14,116,144,.18);
  --hero-bg-a:      #ddeeff;
  --hero-bg-b:      #cce4f8;
  --hero-bg-c:      #d8eef8;
  --hero-border:    rgba(14,116,144,.14);
  --glass-bg:       rgba(255,255,255,.72);
  --glass-border:   rgba(14,116,144,.14);
  --slbl-color:     rgba(14,116,144,.8);
  --slbl-line:      rgba(14,116,144,.18);
  --upload-bg:      rgba(224,242,254,.9);
  --upload-border:  rgba(14,116,144,.70);
  --upload-text:    #0e7490;
  --upload-btn-bg:  linear-gradient(135deg,#1e40af,#0891b2);
  --upload-btn-txt: #ffffff;
  --rb-bg:          rgba(255,255,255,.70);
  --disc-bg:        rgba(245,158,11,.07);
  --disc-border:    rgba(180,120,0,.25);
  --disc-text:      rgba(120,80,0,.85);
  --hr-color:       rgba(14,116,144,.12);
  --tab-bg:         rgba(224,240,252,.70);
  --tab-border:     rgba(14,116,144,.15);
  --tab-color:      rgba(15,30,54,.50);
  --metric-bg:      rgba(255,255,255,.75);
  --metric-border:  rgba(14,116,144,.12);
  --metric-lbl:     rgba(15,30,54,.45);
  --toggle-bg:      rgba(255,237,180,.55);
  --toggle-border:  rgba(200,150,0,.35);
  --toggle-text:    #92400e;
  --toggle-icon:    "☀️";
  --toggle-label:   "Light Mode";
  --hm-bg:          rgba(240,248,255,.95);
  --hm-border:      rgba(14,116,144,.18);
  --hm-header:      rgba(224,240,252,1);
  --pred-bg-a:      rgba(210,235,255,.85);
  --pred-bg-b:      rgba(195,225,250,.90);
}

/* ==========================================
   APPLY VARIABLES TO APP
   ========================================== */
.theme-dark .stApp,
.theme-light .stApp {
...
.theme-dark .stApp {
  ...
  background-image:
    radial-gradient(ellipse 90% 55% at 10% -5%, var(--bg-grad-a) 0%, transparent 45%),
    radial-gradient(ellipse 70% 45% at 95% 105%, var(--bg-grad-b) 0%, transparent 45%),
    radial-gradient(ellipse 50% 70% at 50% 50%, var(--bg-grad-c) 0%, transparent 70%);
...
.topnav{
  ...
  background:var(--nav-bg, rgba(4,9,15,.88));
  backdrop-filter:blur(20px) saturate(180%);
  -webkit-backdrop-filter:blur(20px) saturate(180%);
  border-bottom:1px solid var(--nav-border, rgba(56,189,248,.12));
...
.nav-name{font-family:'Space Grotesk',sans-serif;font-size:16px;font-weight:600;
  color:var(--text-primary, #f8fafc); letter-spacing:-.3px}
.nav-name span{color:#38bdf8}
.nav-tagline{font-family:'DM Mono',monospace;font-size:8.5px;
  color:var(--text-muted, rgba(255,255,255,.28)); letter-spacing:.15em; text-transform:uppercase; margin-top:2px}
...
.hero{
  ...
  background:linear-gradient(130deg,var(--hero-bg-a,#040b1a) 0%,var(--hero-bg-b,#07142e) 50%,var(--hero-bg-c,#040c1e) 100%);
  border-bottom:1px solid var(--hero-border, rgba(56,189,248,.09));
...
.hero-h1{font-family:'Space Grotesk',sans-serif;font-size:2.5rem;font-weight:700;
  color:var(--text-primary, #f8fafc);letter-spacing:-.7px;line-height:1.15;margin-bottom:.55rem}
.hero-h1 .grad{
  background:linear-gradient(92deg,#38bdf8 0%,#818cf8 50%,#a78bfa 100%);
  -webkit-background-clip:text;-webkit-text-fill-color:transparent;background-clip:text}
.hero-desc{font-size:13.5px;color:var(--text-muted, rgba(255,255,255,.42));line-height:1.72;max-width:520px}
...
/* === GLASS CARD === */
.glass{
  background:var(--glass-bg, rgba(255,255,255,.028));
...
/* === SECTION LABEL === */
.slbl{font-family:'DM Mono',monospace;font-size:9.5px;color:var(--slbl-color, rgba(56,189,248,.55));
  text-transform:uppercase;letter-spacing:.17em;margin-bottom:10px;
  display:flex;align-items:center;gap:8px}
.slbl::after{content:'';flex:1;height:1px;background:var(--slbl-line, rgba(56,189,248,.12))}
...
/* === REPORT BLOCKS === */
.rb{border-left:3px solid rgba(147,197,253,.4);background:var(--rb-bg, rgba(255,255,255,.025));
...
.rb-t{font-family:'DM Mono',monospace;font-size:9px;color:var(--text-dim, rgba(255,255,255,.32));
  text-transform:uppercase;letter-spacing:.14em;margin-bottom:6px}
.rb-b{font-size:13px;line-height:1.85;color:var(--text-muted, rgba(255,255,255,.68))}

/* === DISCLAIMER === */
.disc{background:var(--disc-bg, rgba(245,158,11,.05));
  border:1px solid var(--disc-border, rgba(245,158,11,.18));
  ...
  color:var(--disc-text, rgba(253,211,77,.65));line-height:1.75;margin-top:18px}
...
.stTabs [data-baseweb="tab-list"]{
  background:var(--tab-bg, rgba(255,255,255,.035)) !important;
  border-radius:10px !important;padding:3px !important;gap:2px !important;
  border:1px solid var(--tab-border, rgba(255,255,255,.06)) !important}
.stTabs [data-baseweb="tab"]{border-radius:7px !important;
  font-family:'DM Mono',monospace !important;font-size:10.5px !important;
  color:var(--tab-color, rgba(255,255,255,.38)) !important;padding:8px 14px !important}
.stTabs [aria-selected="true"]{
  background:rgba(56,189,248,.14) !important;color:#38bdf8 !important;
  box-shadow:none !important}

[data-testid="stFileUploader"]{
  border:2px dashed var(--upload-border, rgba(56,189,248,.55)) !important;
  border-radius:14px !important;
  background:var(--upload-bg, rgba(20,80,130,.22)) !important;
  transition: background .35s, border-color .35s;
}
...
[data-testid="stFileUploader"] div{
  color: var(--upload-text, #7dd3fc) !important;
}
...
[data-testid="stFileUploader"] [data-testid="baseButton-secondary"]{
  background: var(--upload-btn-bg, linear-gradient(135deg,#1e40af,#0e7490)) !important;
  color: var(--upload-btn-txt, #ffffff) !important;
  ...
  box-shadow: 0 4px 18px rgba(14,116,144,.45), 0 0 0 1px rgba(56,189,248,.2) !important;
...
[data-testid="stFileUploader"] [data-testid="baseButton-secondary"]:hover{
  ...
  box-shadow: 0 8px 28px rgba(14,116,144,.6), 0 0 0 1px rgba(56,189,248,.35), 0 0 30px rgba(56,189,248,.12) !important;
  filter: brightness(1.1) !important;
}

/* Theme toggle button — styled as a chip, NOT as a full-width CTA */
button[data-testid="baseButton-secondary"][kind="secondary"]:has-text,
...
/* Target the theme toggle specifically by its container */
.theme-toggle-col button,
.theme-toggle-col .stButton > button {
  background: var(--toggle-bg, rgba(56,189,248,.10)) !important;
  border: 1px solid var(--toggle-border, rgba(56,189,248,.30)) !important;
  color: var(--toggle-text, #38bdf8) !important;
...
[data-testid="stMetric"]{background:var(--metric-bg, rgba(255,255,255,.04)) !important;
  border:1px solid var(--metric-border, rgba(255,255,255,.07)) !important;border-radius:12px !important;
  padding:12px 15px !important}
[data-testid="stMetricLabel"]{color:var(--metric-lbl, rgba(255,255,255,.38)) !important;font-size:11px !important}
...
[data-testid="stToggle"] label{color:var(--text-muted, rgba(255,255,255,.55)) !important;font-size:13px !important}
...
hr{border-color:var(--hr-color, rgba(255,255,255,.06)) !important;margin:1.8rem 0 !important}
...
# ================================================================
# THEME: inject wrapper class so CSS variables apply
# ================================================================
_theme = st.session_state.get("theme", "dark")
_theme_class = "theme-dark" if _theme == "dark" else "theme-light"
st.markdown(f"""
<script>
(function() {{
  var cls = '{_theme_class}';
  function applyTheme() {{
    var targets = [
...
_toggle_label = "Light Mode" if _theme == "dark" else "Dark Mode"

# Render nav HTML — toggle button rendered separately via Streamlit column
st.markdown(f"""
<div class="topnav">
  <div class="nav-brand">
  ...
  <div class="nav-chips" style="display:flex;align-items:center;gap:8px;flex-wrap:wrap;">
...
# Theme toggle — floated into the nav area via sticky positioning trick
_tog_col1, _tog_col2, _tog_col3 = st.columns([1, 6, 1])
with _tog_col3:
    st.markdown(f"""
    <style>
    /* Pull the toggle row up into the nav */
    ...
    button[data-testid="baseButton-secondary"]#theme_toggle,
    div[data-testid="stColumn"]:last-child .stButton > button {{
      background: {'rgba(56,189,248,.12)' if _theme=='dark' else 'rgba(255,237,180,.55)'} !important;
      border: 1px solid {'rgba(56,189,248,.35)' if _theme=='dark' else 'rgba(200,150,0,.40)'} !important;
      color: {'#7dd3fc' if _theme=='dark' else '#92400e'} !important;
      font-family: 'DM Mono', monospace !important;
      font-size: 10px !important;
      font-weight: 500 !important;
    ...
    div[data-testid="stColumn"]:last-child .stButton > button:hover {{
      ...
      box-shadow: 0 4px 14px {'rgba(56,189,248,.2)' if _theme=='dark' else 'rgba(200,150,0,.2)'} !important;
    }}
