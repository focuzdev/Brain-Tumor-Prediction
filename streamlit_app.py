...
Model: ResNet50V2 (best individual from ResNet50V2 + MobileNetV2 ensemble)
Preprocessing: resnet_v2.preprocess_input  (NOT rescale=1/255)
...
# Constants
# ──────────────────────────────────────────────────────────────────────────────
# CRITICAL: This order must exactly match the CLASS_NAMES / class_indices used
# during training (notebook Cell 2: ["glioma","meningioma","notumor","pituitary"]).
# The trained model outputs probabilities in this exact order.
...
def preprocess(pil_img):
    """
    Preprocess an image for ResNet50V2 inference.

    CRITICAL FIX: The model was trained with
      resnet_v2.preprocess_input()  (scales to [-1, 1] via ImageNet stats)
    NOT with rescale=1/255.
    Using the wrong normalization was the root cause of ~35% accuracy.
    """
    img = pil_img.convert("RGB").resize(IMG_SIZE)
    arr = np.array(img, dtype=np.float32)
    # Apply ResNet50V2 preprocessing (matches training exactly)
    if resnet_preprocess is not None:
        arr = resnet_preprocess(arr)
    else:
        # Fallback if TF not fully available — approximate ImageNet centering
        arr = arr / 127.5 - 1.0
...
def make_gradcam(model, img_array, pred_index):
    ...
    The saved model has structure:  Input → ResNet50V2 backbone → GAP → Dense head
    The backbone is model.layers[1] (the second layer after the Input layer).
    We search for the last Conv2D *inside* the backbone sub-model to get a
    spatially rich feature map.
    """
    # Locate the backbone (ResNet50V2) sub-model
    backbone = None
    for layer in model.layers:
        if hasattr(layer, "layers"):   # it's a sub-model / functional layer
            backbone = layer
            break

    # Find the last Conv2D inside the backbone
    last_conv_name = None
    ...
    # Fall back to searching the top-level model if backbone search failed
    ...
    try:
        # Build grad model: outputs [last_conv_output, final_predictions]
        if backbone is not None:
            conv_output = backbone.get_layer(last_conv_name).output
...
def overlay_gradcam(pil_img, heatmap, alpha=0.55):
    """
    Produce a Grad-CAM overlay that matches clinical publication style:
    - Heatmap is smoothed with a Gaussian to avoid blocky artefacts
    - Jet colormap is applied to the full [0,1] range
    - Alpha blending is stronger (0.55) so hot-spots are clearly visible
    - The original MRI is converted to grayscale-ish (desaturated) so the
      colour heatmap pops — matching the reference image style where the
      brain background is muted and the red/yellow regions dominate.
    """
    orig = np.array(pil_img.convert("RGB").resize(IMG_SIZE), dtype=np.float32)

    # Smooth the raw heatmap to remove block artefacts from low-res conv output
    ...
    # Apply jet colormap
    hm_color = (cm.jet(hm)[:, :, :3] * 255).astype(np.float32)

    # Slightly desaturate the original MRI so the heatmap colour reads clearly
    gray = np.mean(orig, axis=2, keepdims=True)
    orig_desat = orig * 0.4 + gray * 0.6   # partial desaturation

    # Alpha blend: more weight on heatmap in high-activation zones
    # Use the heatmap value itself as a spatially-varying alpha mask
    alpha_mask = alpha + (1 - alpha) * hm[..., np.newaxis] * 0.5
    alpha_mask = np.clip(alpha_mask, 0, 1)

    blend = orig_desat * (1 - alpha_mask) + hm_color * alpha_mask
    blend = np.clip(blend, 0, 255).astype(np.uint8)
    return Image.fromarray(blend)


def make_heatmap_figure(heatmap, pred_class, confidence):
    """
    Render the standalone jet heatmap with a colorbar and annotation,
    styled to match publication figures.
    """
    fig, ax = plt.subplots(figsize=(3.5, 3.5))
    hm_smooth = cv2.GaussianBlur(heatmap.astype(np.float32), (15, 15), 0)
    if hm_smooth.max() > 0:
        hm_smooth /= hm_smooth.max()
    im = ax.imshow(hm_smooth, cmap="jet", vmin=0, vmax=1, interpolation="bilinear")
    ax.axis("off")
    ax.set_facecolor("#0a0a0a")
    fig.patch.set_facecolor("#0a0a0a")
    cb = fig.colorbar(im, ax=ax, fraction=0.040, pad=0.03)
    cb.ax.tick_params(colors="#aaaaaa", labelsize=7)
    cb.set_label("Activation intensity", color="#aaaaaa", fontsize=7, labelpad=6)
    ax.set_title(
        f"AI Attention Map\n{pred_class}  ·  {confidence:.1f}%",
        color="#dddddd", fontsize=8, pad=6, fontweight="bold"
    )
...
def pred_card_html(pred_class, confidence, risk_label, risk_css, dot_css):
    ...
    <div class="pred-container">
      <div class="pred-eyebrow">ResNet50V2 · Predicted Diagnosis</div>
  ...
  <div class="masthead-inner">
    <div class="masthead-top">
      ...
      <div class="masthead-badges">
        <span class="mbadge mbadge-blue">ResNet50V2</span>
    ...
    <div class="masthead-stats">
      ...
      <div class="mstat"><div class="mstat-val">95–97%</div><div class="mstat-label">Test Accuracy</div></div>
      <div class="mstat"><div class="mstat-val">XAI</div><div class="mstat-label">Explainability</div></div>
      <div class="mstat"><div class="mstat-val">v3.0</div><div class="mstat-label">Model Version</div></div>
    </div>
  </div>
</div>
""", unsafe_allow_html=True)
...
if analyze_clicked and pil_image:
    ...
    with st.spinner("Running CNN inference…"):
        ...
        if model:
            ...
            # Demo mode — fake probabilities aligned to CLASS_NAMES order:
            # index 0=Glioma, 1=Meningioma, 2=No Tumor, 3=Pituitary Tumor
            demo_map = {
    ...
    if show_gradcam and gradcam_overlay:
        ...
        <div style="display:flex;align-items:center;gap:18px;margin-bottom:14px;
                    padding:10px 16px;background:#ffffff;border:1px solid #dde5f0;
                    border-radius:10px;flex-wrap:wrap;">
          <span style="font-family:'DM Mono',monospace;font-size:10px;color:#7a96bb;
                       text-transform:uppercase;letter-spacing:0.12em;">Heatmap key</span>
          <span style="display:flex;align-items:center;gap:6px;">
            <span style="width:14px;height:14px;border-radius:3px;
                         background:#d00000;display:inline-block;"></span>
            <span style="font-size:12px;color:#3a5070;">Red / Yellow — highest AI attention</span>
          </span>
          <span style="display:flex;align-items:center;gap:6px;">
            <span style="width:14px;height:14px;border-radius:3px;
                         background:#00b4d8;display:inline-block;"></span>
            <span style="font-size:12px;color:#3a5070;">Cyan / Blue — low attention</span>
          </span>
          <span style="font-family:'DM Mono',monospace;font-size:10px;color:#7a96bb;">
            🔥 Red/yellow regions = high AI attention (likely tumor zone)
          </span>
        </div>""", unsafe_allow_html=True)
        ...
        with gc1:
            st.markdown("""<div style="font-family:'DM Mono',monospace;font-size:10px;
                color:#7a96bb;text-transform:uppercase;letter-spacing:0.1em;
                margin-bottom:6px;">Original MRI</div>""", unsafe_allow_html=True)
            st.image(pil_image, use_column_width=True)

        with gc2:
            st.markdown("""<div style="font-family:'DM Mono',monospace;font-size:10px;
                color:#7a96bb;text-transform:uppercase;letter-spacing:0.1em;
                margin-bottom:6px;">🔥 Grad-CAM Heatmap Overlay</div>""", unsafe_allow_html=True)
            st.image(gradcam_overlay, use_column_width=True)
            st.markdown("""<div style="font-family:'DM Mono',monospace;font-size:10px;
                color:#7a96bb;text-align:center;margin-top:4px;">
                Red/yellow regions = high AI attention</div>""", unsafe_allow_html=True)

        with gc3:
            if heatmap is not None:
                st.markdown("""<div style="font-family:'DM Mono',monospace;font-size:10px;
                    color:#7a96bb;text-transform:uppercase;letter-spacing:0.1em;
                    margin-bottom:6px;">Activation Intensity Map</div>""", unsafe_allow_html=True)
                fig2 = make_heatmap_figure(heatmap, pred_class, confidence)
                st.pyplot(fig2, use_container_width=True)
                plt.close()

        # ── Interpretation note ──────────────────────────────────────────────
        st.markdown(f"""
        <div class="rblock rblock-ok" style="margin-top:10px;">
          <div class="rblock-title">🔥 Grad-CAM Interpretation</div>
          <div class="rblock-body">
            The heatmap highlights the brain regions that most influenced the
            <strong>{pred_class}</strong> prediction. Red and yellow zones indicate
            the highest model attention — these correspond to the suspected tumor region.
            Cyan and blue zones contribute minimally to the decision.
            Clinically meaningful activation should localise to the lesion area rather
            than the skull margin or background.
          </div>
        </div>""", unsafe_allow_html=True)
