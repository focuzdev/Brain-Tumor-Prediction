...
[data-baseweb="menu"] [role="option"]{{background:transparent !important;color:{"#e2e8f0" if _dk else "#0a1628"} !important;-webkit-text-fill-color:{"#e2e8f0" if _dk else "#0a1628"} !important;font-family:'Space Grotesk',sans-serif !important;font-size:14px !important;font-weight:500 !important;padding:12px 18px !important;border-bottom:1px solid {"rgba(255,255,255,.05)" if _dk else "#e8f4fb"} !important;transition:background .15s !important}}
[data-baseweb="menu"] [role="option"]:hover{{background:{"rgba(37,99,235,.18)" if _dk else "rgba(14,116,144,.10)"} !important;color:{"#38bdf8" if _dk else "#0e6b8a"} !important;-webkit-text-fill-color:{"#38bdf8" if _dk else "#0e6b8a"} !important}}
[data-baseweb="menu"] [aria-selected="true"]{{background:{"rgba(37,99,235,.25)" if _dk else "rgba(14,116,144,.14)"} !important;color:{"#38bdf8" if _dk else "#0e6b8a"} !important;-webkit-text-fill-color:{"#38bdf8" if _dk else "#0e6b8a"} !important;font-weight:600 !important}}
</style>
...
def load_models():
    """Load both models and auto-detect class mappings."""
    models = {"resnet": None, "mobilenet": None, "resnet_map": list(range(4)), "mobilenet_map": list(range(4))}
    ...
    if not os.path.exists(MODEL_PATH) and GDRIVE_ID:
        try:
            ...
            st.error(f"Failed to download ResNet50V2: {e}")
    ...
    if os.path.exists(MODEL_PATH):
        try:
            ...
            st.error(f"Failed to load ResNet50V2: {e}")
    ...
    if not os.path.exists(MOBILENET_PATH) and GDRIVE_MOBILENET_ID:
        try:
            ...
            st.error(f"Failed to download MobileNetV2: {e}")
...
def detect_class_mapping(model, model_name):
    """
    Detect the correct class index mapping by testing on sample images.
    Returns a list where map[model_output_idx] = class_names_idx
    """
...
# ================================================================
# FIXED MRI VALIDATION (Relaxed for external datasets)
# ================================================================
def validate_mri(pil_img, strict=False):
    """MRI validator with optional strict mode."""
    import numpy as np
    import cv2
...
def mri_gate_ui(is_valid, confidence, reasons, _dk):
    """Display MRI validation results."""
...
<div style="display:flex;gap:10px;align-items:flex-start;padding:7px 0;
  border-bottom:1px solid {"rgba(255,255,255,.05)" if _dk else "#f0e8e8"};">
...
<div style="background:rgba(239,68,68,.08);border:1px solid rgba(239,68,68,.40);
  border-left:4px solid #ef4444;border-radius:12px;padding:1.1rem 1.4rem;margin-bottom:1rem;">
...
# ================================================================
# FIXED INFERENCE WITH ENSEMBLE
# ================================================================
def run_inference(img, models, temperature=1.4, use_ensemble=True):
    """Run inference with proper class mapping and ensemble."""
    ...
    if resnet_model is not None:
        try:
            ...
            raw_resnet = None
            st.warning(f"ResNet50V2 failed: {e}")
    else:
        raw_resnet = None
    ...
    if mobilenet_model is not None and use_ensemble:
        try:
            ...
            raw_mobilenet = None
            st.warning(f"MobileNetV2 failed: {e}")
    ...
    if raw is not None:
        ...
        if std_px > 55 and mean_px < 80:
            preds = np.array([0.78, 0.12, 0.06, 0.04])
        elif mean_px > 100 and std_px > 45:
            preds = np.array([0.06, 0.82, 0.07, 0.05])
        elif std_px < 35:
            preds = np.array([0.04, 0.03, 0.91, 0.02])
        else:
            preds = np.array([0.05, 0.07, 0.05, 0.83])
        is_demo = True
...
# AI REPORT
# ================================================================
def ai_report(pil_img, pred_class, conf, heatmap_img=None, all_probs=None,
              ensemble_mode="ResNet50V2 single model", is_close_call=False,
              second_cls=None, second_conf=None):
    ...
    try:
        ...
        key = ""
    if not key:
        return mock_report(pred_class, conf), False, "No API key configured."
...
- You are given the CNN's full probability distribution across all 4 classes.
...
- Never invent precise measurements (e.g. "2.8 cm") - use qualitative descriptors.
...
JSON schema:
{
  ...
  "disclaimer": "AI-assisted decision support only. All findings must be confirmed by a licensed radiologist or neurosurgeon."
    ...
    try:
        ...
        return mock_report(pred_class, conf), False, str(e)
...
def mock_report(pc, c):
    T = {
        "Glioma": {
            ...
            "model_reasoning": f"Glioma ({c:.1f}%) supported by ring-enhancing pattern, heterogeneous signal, and peritumoral edema.",
            ...
            "image_quality": "GOOD",
            "image_quality_notes": "T1-weighted axial sequence. Good spatial resolution.",
            "uncertainty_factors": "Partial ambiguity at tumor-edema boundary.",
            "differential_diagnosis": "1. High-grade glioblastoma (GBM). 2. Metastatic lesion.",
            "reliability_score": 91,
            "overall_reliability": "High reliability. Minor uncertainty at infiltrative margin.",
            "disclaimer": "AI-assisted decision support only. All findings must be confirmed by a licensed radiologist or neurosurgeon."
        },
        "Meningioma": {
            ...
            "gradcam_analysis": "Model focuses on the lesion-dura interface and dural tail.",
            "risk_level": "MODERATE",
            "risk_justification": "Most meningiomas are WHO Grade I (benign). Risk depends on size and location.",
            ...
            "image_quality": "GOOD",
            "image_quality_notes": "T1-weighted axial sequence. Adequate resolution.",
        ...
        "Pituitary Tumor": {
            "clinical_interpretation": "Intrasellar mass expanding the sella turcica with suprasellar extension. Optic chiasm displaced superiorly.",
        ...
        "No Tumor": {
            ...
            "location_morphology": "No focal lesion. Gray-white matter differentiation preserved. Midline structures central.",
            "model_reasoning": f"No Tumor ({c:.1f}%) consistent with symmetric architecture, no mass effect.",
            ...
            "patient_explanation": "Good news - the AI did not detect a tumor. The brain scan appears normal.",
            "next_steps": "Clinical follow-up if symptomatic. Repeat imaging if clinically indicated.",
            "image_quality": "GOOD",
            "image_quality_notes": "T1-weighted axial sequence. Good image quality.",
...
with st.sidebar:
    ...
    if _key:
        ...
        st.error("No API key - template reports")
...
with col_out:
    st.markdown(f'''<div class="slbl">
      <span id="ns-result-label">Model Output - Prediction</span>
    </div>''', unsafe_allow_html=True)
...
""", unsafe_allow_html=True)
    ...
    with col_out:
        st.markdown(f"""
<div style="background:{"rgba(56,189,248,.08)" if _dk else "rgba(14,116,144,.07)"};
  border:1px solid {"rgba(56,189,248,.30)" if _dk else "#4a9ab8"};
  border-left:4px solid #38bdf8; border-radius:12px; padding:12px 16px; margin-bottom:16px;
  display:flex; align-items:center; gap:12px;">
...
""", unsafe_allow_html=True)
    ...
    with col_out:
        if conf < 55.0:
            st.warning(f"⚠️ **Low Confidence ({conf:.1f}%)** — Model uncertain. Specialist review essential.")
        elif _glioma_men_ambiguous:
            st.warning(f"⚠️ **Glioma / Meningioma Ambiguity** — Contrast-enhanced MRI recommended.")
        elif is_close_call:
            st.info(f"ℹ️ **Close Call** — {pcls} ({conf:.1f}%) vs {_second_cls} ({_second_conf:.1f}%)")
        if _mri_conf < 0.70:
            st.info(f"ℹ️ MRI gate confidence: {int(_mri_conf*100)}% (borderline)")

    if src == "upload":
        with col_out:
            st.info("📌 **External image notice** — Different scanner/protocol may affect accuracy.")
    ...
    def class_bar_html(name, prob, is_top, color):
        pct = prob * 100
        w_pct = max(pct, 0.5)
        lbl_top = "font-weight:700;color:#f8fafc;" if _dk else "font-weight:700;color:#0a1628;"
        lbl_rest = "font-weight:400;color:rgba(255,255,255,.55);" if _dk else "font-weight:500;color:#2d4a6b;"
        bold = lbl_top if is_top else lbl_rest
        track = "background:rgba(255,255,255,.07);" if _dk else "background:#b8d8ec;"
        fill_rst = "background:rgba(255,255,255,.18);" if _dk else "background:#7ab8d0;"
        fill = f"background:linear-gradient(90deg,{color},{color}cc);" if is_top else fill_rst
        pct_top = "color:#38bdf8;" if _dk else "color:#0e6b8a;"
        pct_rst = "color:rgba(255,255,255,.45);" if _dk else "color:#4a6580;"
...
<div style="margin-bottom:20px;">
  ...
  <div style="font-family:'DM Mono',monospace;font-size:11px;{pct_col}">{pct:.1f}%</div>
    ...
    note = "Synthetic Demo" if is_demo else "GradientTape"
    _eyebrow = ensemble_mode
...
<div class="hm-section">
  <div class="hm-header">
    <div>
      ...
      <div class="hm-sub">{note} | Pure CNN GradientTape | No VLM Required</div>
    </div>
    <div class="hm-legend">
  ...
  <div class="hm-explain">
    ...
    <div class="hm-exp-grid">
      <div class="hm-exp-item">
        ...
        <div class="hm-exp-b">The 224x224 MRI passes through ResNet50V2. The last conv layer outputs a 7x7 feature map with 2048 channels.</div>
      </div>
      <div class="hm-exp-item">
        <div class="hm-exp-t">Step 2 - Gradient Backprop</div>
        <div class="hm-exp-b">TensorFlow GradientTape records how strongly each feature map channel contributed to the "{pcls}" score.</div>
      </div>
      <div class="hm-exp-item">
        <div class="hm-exp-t">Step 3 - Weighted Sum + ReLU</div>
        <div class="hm-exp-b">Each feature map is multiplied by its weight, summed, and ReLU-activated. The coarse 7x7 result is upscaled to 224x224.</div>
      </div>
      <div class="hm-exp-item">
        <div class="hm-exp-t">This Scan</div>
        <div class="hm-exp-b">Mean: <strong style="color:#38bdf8">{mean_a:.3f}</strong>, Peak: <strong style="color:#38bdf8">{max_a:.3f}</strong>, P90: <strong style="color:#38bdf8">{p90_a:.3f}</strong>. {focus_p:.1f}% of pixels exceed 0.5.</div>
      </div>
    </div>
  </div>
</div>
""", unsafe_allow_html=True)
    ...
    if report_is_live:
        ...
        st.warning("Template report shown — NOT a live Claude analysis." + (f" Reason: {report_err}" if report_err else ""))
    ...
    if report_is_live and report.get("agrees_with_cnn") is False:
        st.error("Claude's independent read disagrees with the CNN — see Model Reasoning tab.")
    ...
    with t2:
        st.markdown(rb("Model Reasoning Alignment", report.get("model_reasoning", "")), unsafe_allow_html=True)
        st.markdown(rb("Grad-CAM Activation Analysis", report.get("gradcam_analysis", ""), "rb-grn"), unsafe_allow_html=True)
  ...
  All findings require review by a licensed radiologist or neurosurgeon.
    ...
    st.download_button(
        ...
        data=json.dumps({
            ...
            "model": "ResNet50V2 + MobileNetV2 Ensemble",
